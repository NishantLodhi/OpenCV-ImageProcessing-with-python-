...
But consider a bimodal image (In simple words, bimodal image is an image whose histogram has two peaks). 
For that image, we can approximately take a value in the middle of those peaks as threshold value, right ? 
That is what Otsu binarization does. So in simple words, it automatically calculates a threshold value from 
image histogram for a bimodal image. 
...
For this, our cv2.threshold() function is used, but pass an extra flag,
 ...
 Then the algorithm finds the optimal threshold value and 
 returns you as the second output, retVal. If Otsu thresholding is not used,
...
"""In first case, I applied global thresholding for a value of 127.
 ...
 then applied Otsu thresholding"""
